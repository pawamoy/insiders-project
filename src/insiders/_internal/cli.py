    ...
    def __call__(self) -> int:
        ...
        with GitHub(self.github_token) as github:
            github.sync_team(
                ...
                privileged_users=set(self.github_privileged_users),
                org_users=self.github_org_users,  # type: ignore[arg-type]
            )
        return 0
...
@dataclass(kw_only=True)
class CommandBacklogList:
    """Command to list the backlog of issues."""

    github_namespaces: An[
        list[str],
        cappa.Arg(num_args=-1, default=cappa.ValueFrom(from_config, "backlog_namespaces")),
        Doc("Namespaces to fetch issues from."),
    ]

    github_org_users: An[
        dict[str, list[str]],
        cappa.Arg(short=False, long=True, default=cappa.ValueFrom(from_config, "github_organization_members")),
        Doc("""A mapping of users belonging to sponsoring organizations."""),
    ...
    github_token: An[
        ...
        cappa.Arg(
            short=False,
            long=True,
            default=cappa.Env("GITHUB_TOKEN") | cappa.ValueFrom(from_config, "github_token"),
        ),
    ...
    polar_token: An[
        ...
        cappa.Arg(
            short=False,
            long=True,
            default=cappa.Env("POLAR_TOKEN") | cappa.ValueFrom(from_config, "polar_token"),
        ),
    ...
    def _parse_sort(arg: str) -> list[Callable]:
        strats = []
        for strat in arg.split(","):
            if not strat.endswith(")"):
                strat += "()"  # noqa: PLW2901
            strats.append(eval(f"Backlog.SortStrategy.{strat}"))  # noqa: S307
        return strats

    sort: An[
        list[Callable],
        cappa.Arg(short="-s", long=True, parse=_parse_sort),
        Doc("Sort strategy."),
    ] = field(default_factory=list)

    def __call__(self) -> int:
        with GitHub(self.github_token) as github:
            sponsors = github.get_sponsors(self.github_org_users)
            if self.polar_token:
                with Polar(self.polar_token) as polar:
                    sponsors += polar.get_sponsors()
                    backlog = get_backlog(self.github_namespaces, github=github, polar=polar, sponsors=sponsors)
            else:
                backlog = get_backlog(self.github_namespaces, github=github, sponsors=sponsors)
        if self.sort:
...
@cappa.command(name="backlog", help="Manage the backlog of issues.")
@dataclass(kw_only=True)
class CommandBacklog:
    """Command to manage the backlog of issues."""

    subcommand: An[cappa.Subcommands[CommandBacklogList], Doc("The selected subcommand.")]
...
if __name__ == "__main__":
    sys.exit(main(["-c", "insiders.toml", "backlog", "list"]))

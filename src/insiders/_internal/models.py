...
class Sponsorship:
    ...
    @property
    def grantees(self) -> set[User]:
        """Set of user accounts affected by this sponsorship."""
        if isinstance(self.account, User):
            return {self.account}
        return {user for user, _ in self.account.users.values()}


@dataclass(kw_only=True, eq=False, frozen=True)
class Sponsors:
    """Sponsorships and users."""
    ...
    def grantees(self) -> set[User]:
        ...
        return {grantee for sponsorship in self.sponsorships for grantee in sponsorship.grantees}
